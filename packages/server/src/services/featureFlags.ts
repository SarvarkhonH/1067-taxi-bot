...
export const FEATURES = [
  ...
  "corestream", // 🚕 B qism P0-4: yadroning holat oqimi (`svc:order`/`svc:loc`). ON = bot yadroga ulanadi,
              // buyurtma o'zgarganda ride sweep'ni DARHOL uyg'otadi (sweep 5/15 s → 20/30 s xavfsizlik
              // to'riga siyraklashadi), Mini App `/api/ride-ws` orqali «o'zgardi» signali va o'z
              // haydovchisining joyini oladi (so'rov 3 s → 20 s). OFF = MIJOZLAR uchun bugungi so'rovlar
  ...
  "honesteta", // 🚕 B qism P0-6: halol kutish vaqti. ON = Mini App «odatda X–Y daq» (Koson'ning o'z safarlaridan,
              // yadro modeli, faqat daqiqa, masofa HECH QACHON); qiymat bo'lmasa raqam yo'q (to'g'ri chiziq
              // taxminiga qaytilmaydi). ⛔ YAKUNIY BACKTEST (≥14 kun ma'lumot, ≈2026-09-29, `eta-backtest.js` →
              // VERDICT: PASS) o'tmaguncha YOQILMAYDI. OFF = bugungi «~N daq» AYNAN. Ega-preview bor.
  "tapreorder", // 🚕 B qism P0-8 (ega qarori Q2): bir bosishda qayta chaqirish. ON = uy ekranida «🔁 Yana: <joy>»,
              // taksi yakunida «Yana <joy>dan» — tasdiq ekranisiz `/api/booking/now`. Tasdiq birinchi
              // safarda va kuniga 4+ bekor qilishda qoladi (server ham tekshiradi). OFF = bugungi oqim AYNAN.
  "typedfast", // 🚕 B qism P0-8: botda yozilgan manzil yadro ishonch bilan aniqlasa (operator taxallusi yoki
              // bitta qat'iy moslik — `/addresses/resolve`) — ro'yxat o'rniga BITTA xabar [Chaqirish][Boshqa joy].
              // Taxmin hech qachon mashinaga aylanmaydi: ishonchsiz bo'lsa bugungi ro'yxat. Ega-preview bor.
  "fastopen", // 🚕 B qism P0-2: taksi ekrani 1 soniyadan tez. ON = Mini App varaqni telefon keshidan darhol
              // chizadi (oxirgi holatda faol safar bo'lmasa), katalog telefonda versiya bilan, pin ostidagi
              // nom lokal hisoblanadi; server `/api/booking/boot` — yadroga bitta so'rov (bootstrap),
              // shahar ma'lumotlari 60 s keshda, Server-Timing. OFF = bugungi ochilish AYNAN. Ega-preview bor.
  "chatlive", // 🚕 B qism P0-7: chatdagi jonli karta. ON = haydovchi qabul qilganda karta chat tepasiga JIMGINA
              // qadaladi (safar tugaganda olinadi), «~1 daqiqada yetib keladi» va «keldi» buyurtmaga BIR
              // marta (appState markerlari). OFF = bugungi karta AYNAN; ega-preview (admin chatlari) bor.
              // Pul yo'q, dispetcherlikka tegmaydi.
  "livecars", // 🚕 B qism P0-3 (ega qarori Q1, 2026-09-17): taksi xaritasida REAL bo'sh mashinalar —
              ...
              // «so'ralmoqda» nuri, ×2 son va 11-pol O'CHADI (Mini App, /api/home, bot kartasi).
              // OFF = bugungi ekran AYNAN. Pul yo'q, dispetcherlikka tegmaydi. Ega-preview bor
              // (taxiPreview) — owner-preview-masks-dark-flags: jonli mijozga alohida yoqiladi.
