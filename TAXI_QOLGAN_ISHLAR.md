...
| K-P | **Operator yordamchi APK'si** (CTI) — server tomoni TAYYOR va kutmoqda: `POST operator/phones/ring` · `.../answered` · `.../ended`, `X-Device-Token` bilan; 1 ta qurilma ro'yxatdan o'tgan. **APK'ning o'zi yo'q** (`apps/` da faqat api, client, driver-android, web). Ishlashga TO'SIQ EMAS — operator raqamni qo'lda yozadi (13 ta qo'ng'iroq shu yo'l bilan yozilgan, `device_id` bo'sh) | 4 | `operator-phones.controller.ts` |
